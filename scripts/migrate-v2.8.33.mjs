...
const ownershipTests = [
  ["tests/saved-views-controller-smoke.mjs", 'const controllerSource = read("src/app/saved-views-controller.js");\n', 'assert.ok(app.includes("SignalDockSavedViewsController.create"), "Saved Views controller factory wiring missing");', 'SignalDockSavedViewsController.create', 'datasetViewComposition.createSavedViews()'],
  ["tests/dataset-filter-controller-smoke.mjs", 'const controllerSource = read("src/app/dataset-filter-controller.js");\n', 'assert.ok(app.includes("SignalDockDatasetFilterController.create"), "Dataset Filter controller factory wiring missing");', 'SignalDockDatasetFilterController.create', 'datasetViewComposition.createDatasetFilter()'],
  ["tests/table-view-controller-smoke.mjs", 'const controllerSource = read("src/app/table-view-controller.js");\n', 'assert.ok(app.includes("SignalDockTableViewController.create"));', 'SignalDockTableViewController.create', 'datasetViewComposition.createTableView()'],
  ["tests/dataset-overview-controller-smoke.mjs", 'const controllerSource = read("src/app/dataset-overview-controller.js");\n', 'assert.ok(app.includes("SignalDockDatasetOverviewController.create"), "Dataset Overview controller factory wiring missing");', 'SignalDockDatasetOverviewController.create', 'datasetViewComposition.createDatasetOverview()'],
  ["tests/view-orchestrator-controller-smoke.mjs", 'const source = read("src/app/view-orchestrator-controller.js");\n', 'assert.ok(app.includes("SignalDockViewOrchestratorController.create"));', 'SignalDockViewOrchestratorController.create', 'datasetViewComposition.createViewOrchestrator()']
];

for (const [file, declarationAnchor, oldAssertion, factoryToken, appToken] of ownershipTests) {
  let test = read(file);
  test = r1(
    test,
    declarationAnchor,
  ...
  test = r1(
    test,
    oldAssertion,
    'assert.ok(compositionSource.includes("' + factoryToken + '"), "composition factory wiring missing: ' + factoryToken + '");\nassert.ok(app.includes("' + appToken + '"), "root composition call missing: ' + appToken + '");',
