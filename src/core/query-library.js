(function (root) {
  ...
  function bulkUpdate(items, ids, patch = {}) { const wanted = new Set((Array.isArray(ids) ? ids : []).map((id) => clean(id, 96))); const now = new Date().toISOString(); return save(normalize(items).map((item, index) => wanted.has(item.id) ? normalizeItem({ ...item, ...patch, id: item.id, createdAt: item.createdAt, updatedAt: now, folder: "folder" in patch ? cleanFolder(patch.folder) : item.folder, tags: "tags" in patch ? cleanTags(patch.tags) : item.tags }, index) : item)); }
  function bulkRemove(items, ids) { const wanted = new Set((Array.isArray(ids) ? ids : []).map((id) => clean(id, 96))); return save(normalize(items).filter((item) => !wanted.has(item.id))); }
