(function (root) {
  ...
  function upsert(items, patch, id = "") { const list = normalize(items); const now = new Date().toISOString(); const index = id ? list.findIndex((item) => item.id === id) : -1; if (index >= 0) list[index] = normalizeItem({ ...list[index], ...patch, id: list[index].id, createdAt: list[index].createdAt, updatedAt: now }, index); else { if (list.length >= MAX_ITEMS) throw new Error(`Query library is limited to ${MAX_ITEMS} items.`); list.unshift(normalizeItem({ ...patch, id: `query-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 7)}`, createdAt: now, updatedAt: now }, list.length)); } return save(list); }
  function remove(items, id) { return save(normalize(items).filter((item) => item.id !== id)); }
  function toggleFavorite(items, id) { const list = normalize(items); const item = list.find((candidate) => candidate.id === id); return item ? upsert(list, { favorite: !item.favorite }, id) : list; }
  function moveToFolder(items, id, folder) { return upsert(items, { folder: cleanFolder(folder) }, id); }
  function markUsed(items, id, when = new Date().toISOString()) { const list = normalize(items); const item = list.find((candidate) => candidate.id === id); return item ? upsert(list, { useCount: item.useCount + 1, lastUsedAt: clean(when, 64) }, id) : list; }
  function duplicate(items, id, patch = {}) { const list = normalize(items); const item = list.find((candidate) => candidate.id === id); if (!item) throw new Error("Query not found."); const { id: _id, createdAt: _createdAt, updatedAt: _updatedAt, useCount: _useCount, lastUsedAt: _lastUsedAt, ...copy } = item; return upsert(list, { ...copy, ...patch, name: clean(patch.name, 120) || `${item.name} copy`, favorite: Boolean(patch.favorite), useCount: 0, lastUsedAt: "" }); }
  function bulkUpdate(items, ids, patch = {}) { const wanted = cleanIdSet(ids); const now = new Date().toISOString(); return save(normalize(items).map((item, index) => wanted.has(item.id) ? normalizeItem({ ...item, ...patch, id: item.id, createdAt: item.createdAt, updatedAt: now, folder: "folder" in patch ? cleanFolder(patch.folder) : item.folder, tags: "tags" in patch ? cleanTags(patch.tags) : item.tags }, index) : item)); }
  function bulkRemove(items, ids) { const wanted = cleanIdSet(ids); return save(normalize(items).filter((item) => !wanted.has(item.id))); }
  ...
  function renameFolder(items, from, to) { const source = cleanFolder(from); const target = cleanFolder(to); return bulkUpdate(items, normalize(items).filter((item) => item.folder === source).map((item) => item.id), { folder: target }); }
  ...
  function exportSelected(items, ids) { const wanted = cleanIdSet(ids); return exportJson(normalize(items).filter((item) => wanted.has(item.id))); }
