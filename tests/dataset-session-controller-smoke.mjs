...
state.filteredIndexes=[0,99,1];r=c.exportFiltered();assert.equal(r.mode,"json");assert.equal(r.count,2);assert.equal(json.p.length,2);
state.filteredIndexes=Array.from({length:50001},(_,i)=>i<2?i:999999);r=c.exportFiltered();assert.equal(r.mode,"ndjson");assert.equal(r.count,2);assert.ok(parts.n.endsWith(".ndjson"));assert.equal(parts.p.length,1);
assert.ok(!source.includes("state.filteredIndexes.map((index) => state.entries[index]).filter(Boolean).map(normalizeExportEntry)"));
