...
const source=fs.readFileSync(new URL('../src/investigation/case-workspace.js',import.meta.url),'utf8');assert(source.includes('function removeCollectionItem('),'case workspace must centralize collection removal');assert((source.match(/\.filter\(\(item\) => item\.id !== id\)/g)||[]).length===1,'case workspace should keep one shared remove filter');assert(source.includes('function evidenceItemIdSet('),'case workspace must centralize evidence id collection');assert(!source.includes('new Set((evidenceItems || []).map'),'case workspace should not duplicate evidence id set construction');
