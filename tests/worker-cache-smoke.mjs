...
const here=path.dirname(fileURLToPath(import.meta.url)); const root=path.resolve(here,'..'); const posted=[]; const records=new Map(); const workerToken='b'.repeat(32); let messageHandler=null;
...
globalThis.self={location:{href:`https://local.invalid/filter-worker.js?sd_session=${workerToken}`},postMessage(message){posted.push(message);},addEventListener(type,handler){if(type==='message')messageHandler=handler;}}; globalThis.importScripts=(...names)=>{for(const name of names)vm.runInThisContext(fs.readFileSync(path.join(root,name),'utf8'),{filename:name});}; globalThis.performance??={now:()=>Date.now()};
vm.runInThisContext(fs.readFileSync(path.join(root,'filter-worker.js'),'utf8'),{filename:'filter-worker.js'}); const handler=messageHandler||self.onmessage; if(typeof handler!=='function')throw new Error('worker message handler not registered'); posted.shift();
...
await handler({data:envelope({type:'index',version:1,entries})}); const first=posted.shift(); if(first?.type!=='indexed')throw new Error('first index failed');
await new Promise(r=>setTimeout(r,20));
await handler({data:envelope({type:'index',version:2,entries})}); const second=posted.shift();
if(!second?.searchIndex?.cacheHit)throw new Error(`expected search cache hit, got ${JSON.stringify(second?.searchIndex)}`);
await handler({data:envelope({type:'filter',requestId:9,request:{query:'needle',showUnknown:true}})}); const filtered=posted.shift();
