...
const metadataHeavy=api.normalize({...B,id:'metadata-heavy',tags:[...Array.from({length:5000},()=> 'dup'),...Array.from({length:25},(_,i)=>`tag-${i}`)],sources:[...Array.from({length:5000},()=>''),...Array.from({length:600},(_,i)=>`source-${i}`)]});
assert.equal(metadataHeavy.tags.length,20);assert.equal(metadataHeavy.tags[19],'tag-18');assert.equal(metadataHeavy.sources.length,500);assert.equal(metadataHeavy.sources[0],'source-0');assert.equal(metadataHeavy.sources[499],'source-499');
