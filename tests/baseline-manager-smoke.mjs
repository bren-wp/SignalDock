...
assert.equal(malformed.services.length,2);assert.deepEqual(JSON.parse(JSON.stringify(malformed.services[0])),{service:'api',entries:5,errors:2,warnings:1,errorRate:0.4,p95Ms:12.5});assert.deepEqual(JSON.parse(JSON.stringify(malformed.services[1])),{service:'broken',entries:0,errors:0,warnings:0,errorRate:0,p95Ms:null});
assert.equal(malformed.dependencies.length,1);assert.deepEqual(JSON.parse(JSON.stringify(malformed.dependencies[0])),{from:'api',to:'db',calls:7,errors:1,errorRate:0.2,p95Ms:9});
assert.equal(malformed.traceSets.length,1);assert.deepEqual(JSON.parse(JSON.stringify(malformed.traceSets[0])),{signature:'api→db',services:['api','db'],traces:3,errorRate:0.5,medianDurationMs:10,p95DurationMs:20,medianSpans:2});
