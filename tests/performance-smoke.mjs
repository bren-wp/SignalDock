...
assert(snapshot.metrics.filter.maxMs === 10 && snapshot.metrics.filter.p95Ms === 10, "filter p95/max mismatch");
assert(snapshot.metrics.render.count === 1 && elapsed >= 0, "timed metric missing");
console.log("PASS rolling performance metrics");
const source = fs.readFileSync(path.join(root, "src/core/performance.js"), "utf8");
assert(!source.includes("const values = samples.map"), "performance summary must not allocate a values array before sorting a second copy");
assert(!source.includes("const sorted = [...values]"), "performance summary must use one value buffer");
